...
class data_generator:
    ...
    def __iter__(self):
        ...
        for index, i in enumerate(idxs):
            ...
            input_id = self.tokenizer.encode(text, text_pair, max_length=self.max_length,truncation='longest_first')
            input_mask = [1] * len(input_id)
            if len(text) + 2 <= len(input_id):  # CLS text1 SEP text2 SEP ,加2是因为CLS和SEP
